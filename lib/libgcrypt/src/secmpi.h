...
#define OCTETS_TO_MPINT(oc, mp, len) \
    mp_read_unsigned_octets((mp), oc, len)
    // FIXME: CHECK_MPI_OK(mp_read_unsigned_octets((mp), oc, len))
 
#define SECITEM_TO_MPINT(it, mp) \
    mp_read_unsigned_octets((mp), (it).data, (it).len)
    // FIXME: CHECK_MPI_OK(mp_read_unsigned_octets((mp), (it).data, (it).len))
 
#define MPINT_TO_SECITEM(mp, it, arena)                         \
 do { int mpintLen = mp_unsigned_octet_size(mp);                \
    SECITEM_AllocItem(arena, (it), mpintLen);                   \
    mp_to_unsigned_octets(mp, (it)->data, (it)->len);           \
  } while (0)
  // do { int mpintLen = mp_unsigned_octet_size(mp);                \
